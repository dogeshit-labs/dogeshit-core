...
contract ShitFountain {
	...
	uint256 public total_stake = 0;
	uint256 public last_reward_block = 0;
	uint256 public reward_per_token = 0;

	mapping(address => uint256) public stake;
	mapping(address => uint256) public reward_tally;
	...
	function deposit_stake(uint256 amount) public {
		...
		reward_tally[msg.sender] = reward_tally[msg.sender] + reward_per_token.multiplyDecimalRound(amount);
	...
	function distribute() public {
		if (total_stake == 0) {
		  ...
		  reward_per_token = reward_per_token + rewards.divideDecimalRound(total_stake);
			*/
			reward_per_token = reward_per_token +
				((block.number - last_reward_block)*reward_per_block()).divideDecimalRound(total_stake);
			last_reward_block = block.number;
		}
	}
	...
	function unclaimed_reward(address account) public view returns (uint256) {
		...
		uint256 blocks = block.number - last_reward_block;
		uint256 rewards = blocks*rpb;
		uint256 _reward_per_token = reward_per_token + rewards.divideDecimalRound(total_stake);
		return stake[account].multiplyDecimalRound(_reward_per_token) - reward_tally[account];
		*/
		return total_stake > 0 ? stake[account].multiplyDecimalRound(
			reward_per_token +
			 ((block.number-last_reward_block)*reward_per_block()).divideDecimalRound(total_stake))
	...
	function compute_reward(address account) public view returns (uint256) {
		return stake[account].multiplyDecimalRound(reward_per_token) - reward_tally[account];
	}
	...
	function withdraw_rewards() public {
		...
		if ( reward > 0 ) {
			reward_tally[msg.sender] = stake[msg.sender].multiplyDecimalRound(reward_per_token);
